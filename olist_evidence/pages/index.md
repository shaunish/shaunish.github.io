...
We don't have the Olist website traffic data to confirm this. But we do have 11.51% of people that were very unhappy with their experience and left a one-star review. Can we figure out what they didn't like, maybe using a little NLP and elbow grease? To investigate, let's calculate the [TD-IDF score](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) for each word and 2-word phrase in every one-star review. We can use the open-source [spaCy](https://spacy.io/) NLP library to clean up the text before we analyze it, and scikit-learn has a great td-idf library. The results are below: the top thirty most frequent terms that show up in one-star reviews (remember that Olist customers, being Brazilian, leave comments in Portuguese).
...
Much of what we see are generic terms like 'produto' (product) and 'comprei' (I bought). But there is a theme that surfaces. We see 'entregar' and 'entrega' (delivery) near the top, 'chegar' (to arrive), 'vir' (to come), 'esperar' and 'aguardar' (to wait), 'correio' (mail), 'enviar' (to send) - all likely references to the delivery of the product. These seem to be more common than complaints about, say, the 'qualidade' (quality) of the product. This isn't definitive proof of anything, but it does [waggle its eyebrows suggestively](https://xkcd.com/552/): are unhappy customers receiving their orders late? Let's take a look!
...
I organized all the data used in this project using the dbt platform and DuckDB - you can check it out [here](https://github.com/shaunish/olist_dbt). All the charts were made using Evidence.dev. If you want to check out the Evidence source code, it's available at my Github [here](https://github.com/shaunish/shaunish.github.io/tree/master/olist). Thanks for reading!